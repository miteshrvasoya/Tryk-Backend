...
export class KBAnswerService {
  ...
  private static readonly DEFAULT_FALLBACK = "I'm not sure about that. Let me connect you with our support team for better assistance.";
  ...
  static async generateGroundedAnswer(
    ...
    const {
      ...
      includeSources = true,
      minConfidence = 0.3,
      fallbackMessage = this.DEFAULT_FALLBACK
    } = options;
   ...
   */
  static buildContext(documents: KBDocument[]): string {
    let context = "CONTEXT:\n\n";
   ...
   */
  static async assessContextRelevance(query: string, context: string): Promise<number> {
    try {
      // Use AI service to check relevance
      const isRelevant = await AIService.checkRelevance(query, context);
...
Provide a concise, helpful answer using ONLY the information in the context above. Maximum ${maxLength} words.`;
      ...
      const response = await AIService.generateCustomerResponse(
        shopName,
        query,
        context,
        null, // No template for KB answers
   ...
   */
  static prepareSources(documents: KBDocument[]): Array<{
    id: string;
    title?: string;
    url?: string;
    snippet: string;
   ...
   */
  static createSnippet(content: string, maxLength: number): string {
   ...
   */
  static calculateOverallConfidence(documents: KBDocument[], contextRelevance: number): number {
   ...
   */
  static async validateAnswer(answer: string, context: string): Promise<boolean> {
    try {
...
Does the answer contain information that is NOT supported by the context? Reply with ONLY "YES" or "NO".`;
   ...
   */
  static async getAnswerStats(shopId: string, days: number = 7): Promise<any> {
   ...
   */
  static formatAnswerWithSources(answer: GroundedAnswer): string {
