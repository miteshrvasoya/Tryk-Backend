...
interface ToolConfig {
  handler: (context: RequestContext, intentResult: IntentResult) => Promise<any>;
...
export class RequestRouterService {
  ...
  private static readonly TOOL_REGISTRY: Record<string, ToolConfig> = {
    order_lookup: {
   ...
   */
  static async routeRequest(context: RequestContext): Promise<{
    ...
    const intentResult = await IntentClassifierService.classifyIntent(
      context.message,
   ...
   */
  private static makeRoutingDecision(intentResult: IntentResult, context: RequestContext): RoutingDecision {
    const { intent, confidence, entities } = intentResult;
    
    // Get recommended tools for this intent
    const recommendedTools = IntentClassifierService.getRecommendedTools(intent);
    
    // Validate intent requirements
    const requirements = IntentClassifierService.validateIntentRequirements(intent, entities);
   ...
   * Order lookup tool
   */
  private static async handleOrderLookup(context: RequestContext, intentResult: IntentResult): Promise<any> {
   ...
   * Product search tool
   */
  private static async handleProductSearch(context: RequestContext, intentResult: IntentResult): Promise<any> {
   ...
   * Knowledge base tool
   */
  private static async handleKnowledgeBase(context: RequestContext, intentResult: IntentResult): Promise<any> {
    // Import dynamically to avoid circular dependencies
    const { KBQueryService } = await import('./kb-query.service');
    const { KBRerankerService } = await import('./kb-reranker.service');

    // Generate embedding for search
    const normalizedQuery = await KBQueryService.processQuery(
      context.shopId, 
      context.message
    );
    ...
    // Rerank results
    const rerankedDocs = await KBRerankerService.rerankByRelevance(
      context.message,
      retrievalResult.documents,
      intentResult.intent as any, // Type conversion for compatibility
      {
        maxResults: 3,
        boostRecent: true,
   ...
   * Escalation tool
   */
  private static async handleEscalation(context: RequestContext, intentResult: IntentResult): Promise<any> {
   ...
   */
  private static generateOrderResponse(orderData: any): any {
    if (!orderData.found) {
      return {
   ...
   */
  private static generateProductResponse(productData: any): any {
   ...
   */
  private static async generateKBResponse(
    context: RequestContext,
    intentResult: IntentResult,
    kbData: any
  ): Promise<any> {
    return await KBAnswerService.generateGroundedAnswer(
      context.shopId,
      context.message,
      kbData.documents,
      intentResult.intent as any // Type conversion for compatibility
   ...
   */
  private static generateEscalationResponse(): any {
    return {
   ...
   */
  static async getToolHealth(): Promise<Record<string, any>> {
