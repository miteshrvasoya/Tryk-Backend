...
export class TemplateService {
    ...
    static async createTemplate(shopId: string, data: any) {
    ...
    static async updateTemplate(id: string, shopId: string, data: any) {
    ...
    static async deleteTemplate(id: string, shopId: string) {
        await query('DELETE FROM response_templates WHERE id = $1 AND shop_id = $2', [id, shopId]);
        return { success: true };
    }

    static async getTemplatesByShop(shopId: string) {
        const res = await query('SELECT * FROM response_templates WHERE shop_id = $1 ORDER BY created_at DESC', [shopId]);
        return res.rows;
    }

    static async findBestTemplate(shopId: string, intent: string) {
