...
export class ChatEngineService {
  ...
  static async processMessage(shopId: string, customerMessage: string, metadata: any = {}) {
    ...
    const convResult = await query(
        `SELECT id, metadata FROM conversations WHERE shop_id = $1 AND customer_id = $2 AND status = 'active' ORDER BY updated_at DESC LIMIT 1`,
        [shopId, customerId]
    );
    ...
    if (convResult.rows.length > 0) {
        ...
        const newConv = await query(
            `INSERT INTO conversations (shop_id, customer_id, status, metadata) VALUES ($1, $2, 'active', '{}') RETURNING id`,
            [shopId, customerId]
        );
    ...
    // Save User Message
    await query(
        `INSERT INTO messages (conversation_id, sender, role, content) VALUES ($1, $2, 'user', $3)`,
        [conversationId, customerId, customerMessage]
    );
...
`;
         ...
         isRelevant = await AIService.checkRelevance(customerMessage, context);
    }
    
    // Override relevance for GREETING (we don't need context for hi)
    if (intent === 'GREETING') isRelevant = true;
  ...
  private static async detectIntent(message: string): Promise<string> {
      // Deprecated in favor of detectIntentWithState
      return (await this.detectIntentWithState(message, {})).intent;
  }

  private static async detectIntentWithState(message: string, state: any): Promise<{ intent: string, data?: any }> {
    ...
    // Product Queries
    if (lowers.match(/\b(price|cost|how much|buy|sell|have|stock|looking for|search)\b/) || lowers.includes('do you have')) return { intent: 'PRODUCT_QUERY' };

    if (lowers.includes('order') || lowers.includes('track') || lowers.includes('where is')) return { intent: 'ORDER_STATUS' }; // General status inquiry
    if (lowers.includes('refund') || lowers.includes('return') || lowers.includes('cancel')) return { intent: 'POLICY' };
    if (lowers.includes('stock') || lowers.includes('available') || lowers.includes('price')) return { intent: 'INVENTORY' };
    if (lowers.match(/\b(hi|hello|hey|greetings)\b/)) return { intent: 'GREETING' };
    
    return { intent: 'GENERAL' };
  }

  private static async searchFAQs(shopId: string, message: string) {
  ...
  private static async fallbackSearch(shopId: string, message: string) {
  ...
  private static calculateConfidence(intent: string, faqMatches: any[], isRelevant: boolean) {
    // Special handling for GREETING -> Always confident
    if (intent === 'GREETING') return 100;
    
    // Order Lookup is usually high confidence if regex matched
    if (intent === 'ORDER_LOOKUP') return 95;
    
    if (!isRelevant) return 0; // AI Judge killed it
    ...
    if (intent === 'ORDER_STATUS') base += 10;
    
    return base;
  }

  private static async generateAIResponse(shopId: string, message: string, context: string, intent: string, history: any[] = []) {
    try {
        ...
        // 2. Context passed in
        const template = await TemplateService.findBestTemplate(shopId, intent);
        ...
        return await AIService.generateCustomerResponse(shopName, message, context || "No specific FAQ found.", template, history);
    } catch (e) {
  ...
  private static async escalate(shopId: string, message: string, faqMatches: any[], reason: string, conversationId: string) {
    ...
    await NotificationService.sendEscalationEmail(shopId, conversationId, reason, message);
  }

  private static mapFriendlyStatus(financial: string, fulfillment: string | null): string {
    const fin = financial?.toLowerCase() || '';
    const ful = fulfillment?.toLowerCase() || 'unfulfilled';

    if (fin === 'refunded' || fin === 'voided') return 'Refunded / Cancelled';
    if (fin === 'pending') return 'Payment Pending';
