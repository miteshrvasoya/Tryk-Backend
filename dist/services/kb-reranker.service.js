...
exports.KBRerankerService = void 0;
class KBRerankerService {
    /**
     * Advanced reranking with multiple scoring factors
     */
    static async rerankByRelevance(query, documents, intent, options = {}) {
        const { maxResults = 10, boostRecent = true, boostTitleMatches = true, boostExactMatches = true, minSimilarity = 0.1 } = options;
     ...
     */
    static async calculateDocumentScore(document, query, intent, options) {
        ...
        const finalScore = (semanticSimilarity * 0.4) + // 40% semantic similarity
     ...
     */
    static calculateKeywordOverlap(query, content) {
     ...
     */
    static calculateIntentRelevance(sourceType, intent) {
        if (!intent)
     ...
     */
    static calculateFreshnessBonus(createdAt) {
        if (!createdAt)
            return 0;
     ...
     */
    static calculateTitleMatchBonus(query, title) {
        if (!title)
            return 0;
        const queryWords = this.tokenize(query);
     ...
     */
    static calculateExactMatchBonus(query, content) {
        // Exact phrase match
     ...
     */
    static diversifyResults(documents, maxSimilarity = 0.8) {
        const diversified = [];
        for (const doc of documents) {
            // Check if document is too similar to already selected documents
            const isTooSimilar = diversified.some(selected => this.calculateDocumentSimilarity(doc, selected) > maxSimilarity);
            if (!isTooSimilar) {
     ...
     */
    static calculateDocumentSimilarity(doc1, doc2) {
     ...
     */
    static explainReranking(scoredDocuments) {
...
exports.KBRerankerService = KBRerankerService;
