...
class ContextBuilderService {
     ...
     */
    static async buildContext(query, intent, toolResults, conversationHistory = [], shopName) {
        ...
        for (const toolResult of toolResults) {
            ...
            if (contextSource) {
                ...
                if (contextSource.type === 'shopify_api')
                    hasShopifyData = true;
                if (contextSource.type === 'knowledge_base')
                    hasKBData = true;
            }
        }
     ...
     */
    static async convertToolResultToSource(toolResult, intent) {
     ...
     */
    static buildOrderSource(orderData) {
        if (!orderData.found) {
            return {
                type: 'shopify_api',
                data: {
...
${orderData.trackingUrl ? `- Tracking: ${orderData.trackingUrl}` : ''}`;
     ...
     */
    static buildProductSource(productData) {
        ...
        const productText = productData.products
            .map((p, i) => `${i + 1}. ${p.title} - $${p.price} (Available: ${p.inventory_quantity || 'Unknown'})`)
            .join('\n');
        return {
            type: 'shopify_api',
            data: {
    ...
    static buildKBSource(kbData) {
        ...
        return {
            ...
            relevanceScore: Math.max(...kbData.documents.map((doc) => doc.similarity || doc.rank || 0)) * 100,
            timestamp: new Date().toISOString()
        };
    }
    /**
     ...
     */
    static buildHistorySource(conversationHistory, intent) {
     ...
     */
    static buildCombinedText(sources, intent, shopName) {
     ...
     */
    static getIntentInstructions(intent) {
        const instructions = {
     ...
     */
    static formatSourceForLLM(source) {
        let formatted = '';
        switch (source.type) {
            case 'shopify_api':
     ...
     */
    static formatShopifySource(data) {
        ...
        if (data.found && data.products) {
            return `PRODUCT INFORMATION:\n${data.products.map((p) => `- ${p.title}: $${p.price} (Stock: ${p.inventory_quantity || 'Unknown'})`).join('\n')}`;
        }
     ...
     */
    static formatKBSource(data) {
        if (!data.found) {
            return 'KNOWLEDGE BASE: No relevant information found';
        }
        return `KNOWLEDGE BASE INFORMATION:\n${data.documents.map((doc, i) => `[Source ${i + 1}] ${doc.title || 'Untitled'}\n${doc.content}`).join('\n\n')}`;
    }
    /**
     * Format conversation history for LLM
     */
    static formatHistorySource(data) {
        return `CONVERSATION HISTORY:\n${data.messages.map((msg) => `${msg.role.toUpperCase()}: ${msg.content}`).join('\n')}`;
    }
    /**
     * Calculate overall confidence from all sources
     */
    static calculateOverallConfidence(sources, toolResults) {
        if (sources.length === 0)
            return 20;
    ...
    static validateContext(context) {
        ...
        const avgRelevance = context.sources.reduce((sum, source) => sum + (source.relevanceScore || 0), 0) / context.sources.length;
        if (avgRelevance < 40) {
