...
class IntentClassifierService {
     ...
     */
    static async classifyIntent(message, conversationHistory = []) {
    ...
    static matchPatterns(message) {
        ...
        for (const [intentType, config] of Object.entries(this.INTENT_PATTERNS)) {
            ...
            // Keyword matching
            const keywordMatches = config.keywords.filter(keyword => message.includes(keyword.toLowerCase())).length;
            if (keywordMatches > 0) {
    ...
    static extractEntities(message, intent) {
        ...
        if (intent === 'product_availability') {
            ...
            const productWords = words.filter(word => !['do', 'you', 'have', 'is', 'in', 'stock', 'available', 'price', 'cost', 'how', 'much', 'the', 'this', 'that'].includes(word.toLowerCase()));
     ...
     */
    static adjustWithContext(result, history) {
        if (history.length === 0)
            return result;
     ...
     */
    static calculateFinalConfidence(result, entities) {
     ...
     */
    static getRecommendedTools(intent) {
        const toolMap = {
     ...
     */
    static validateIntentRequirements(intent, entities) {
        const requirements = {
...
IntentClassifierService.INTENT_PATTERNS = {
    order_status: {
        patterns: [
